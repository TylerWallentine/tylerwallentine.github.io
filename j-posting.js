...
class PostEditor {
    ...
    const buttons = document.querySelectorAll('.toolbar-btn:not([data-action="preview"]):not([data-action="export"])');
    const inputs = document.querySelectorAll('.font-family, .color-picker');
        ...
        try {
            ...
            if (saved) {
                ...
                if (content.isMarkdownMode) {
                    const markdownBtn = document.querySelector('[data-action="markdown"]');
...
class PostFirebaseIntegration {
    ...
    addMetadataForm() {
        ...
        metadataForm.innerHTML = `
            ...
            <div class="category-input-container">
                <select id="post-category" style="padding:8px; border:1px solid #ccc; border-radius:4px; width:100%;">
                <option value="">Select a category...</option>
                </select>
                <input type="text" id="new-category" placeholder="Or create new category" style="margin-top:8px; padding:8px; border:1px solid #ccc; border-radius:4px; width:100%;">
            </div>
            </div>
    ...
    try {
        ...
        const postsSnapshot = await window.fsGetDocs(window.fsCollection(db, 'posts'));
        const categories = new Set();

        postsSnapshot.forEach(doc => {
        const data = doc.data();
        if (data.category && data.published) categories.add(data.category);
        });
        ...
        console.warn('Could not populate category dropdown:', err);
    }
    }
    ...
    async savePost(publish = false) {
        ...
        const categoryInput = document.getElementById('post-category').value.trim();
        const newCategoryInput = document.getElementById('new-category').value.trim();
        const category = newCategoryInput || categoryInput || 'Uncategorized';
...
// Initialize when PostEditor is ready
document.addEventListener('DOMContentLoaded', () => {
    // Wait for PostEditor to be initialized
    const checkForEditor = setInterval(() => {
        if (window.postEditor) {
            clearInterval(checkForEditor);
            window.postIntegration = new PostFirebaseIntegration(window.postEditor);
        }
    }, 100);
});
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (editor.loadFromLocalStorage()) {
        editor.showNotification('Previous content restored!');
    }
